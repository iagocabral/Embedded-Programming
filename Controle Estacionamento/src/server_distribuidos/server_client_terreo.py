...
def request_status_lotacao_B(server_host, server_port):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        sock.connect((server_host, server_port))
        request = json.dumps({'tipo': 'status_lotacao_B'})
        sock.sendall(request.encode('utf-8'))
        response = sock.recv(1024)
        status = json.loads(response.decode('utf-8'))
...
def request_status_lotacao_A_v(server_host, server_port):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        sock.connect((server_host, server_port))
        request = json.dumps({'tipo': 'status_lotacao_A_v'})
        sock.sendall(request.encode('utf-8'))
        response = sock.recv(1024)
        status = json.loads(response.decode('utf-8'))
        return status.get('lotar_A', False)
    
def request_status_lotacao_B_v(server_host, server_port):
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
        sock.connect((server_host, server_port))
...
if __name__ == "__main__":
    listener_thread = threading.Thread(target=listen_for_updates, args=('localhost', 10400), daemon=True)
