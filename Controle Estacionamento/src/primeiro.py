...
import json
from server_distribuidos.server_client_1 import send_data, enviar_carro_para_servidor, request_car_info, update_car_info, request_remove_car, listen_for_updates, request_status_lotacao_A, request_status_lotacao_A_v
from terreo import controlar_saida

endereco_01 = LED(13)
endereco_02 = LED(6)
endereco_03 = LED(5)
sensor_de_vaga = Button(20)
sinal_de_lotado_fechado = LED(8)
sensor_de_passagem_1 = Button(16)
sensor_de_passagem_2 = Button(21)
...
def sensor_2_activated_callback():
    ...
    if not waiting_for_sensor_2:
        ...
        mensagem_subindo = ("Carro subindo para o primeiro andar.")
        waiting_for_sensor_2 = False
    send_data('localhost', 10399, to_json_message(mensagem_subindo))
    waiting_for_sensor_2 = False
...
def processar_mudanca_vaga(vaga_id, estado):
    ...
    if estado_str == "livre" and comeco > 8:
        ...
        car = request_car_info('localhost', 10399, vaga_info)
        vaga_info = {'tipo': tipo, 'A': vaga_num}
        car['dados']['vaga'] = vaga_info
        car['dados']['vaga_num'] = vaga_num
        update_car_info('localhost', 10399, car)
        tem_vaga[vaga_id] = False

    estacionamento_lotado = all(not vaga for vaga in tem_vaga)

    verifica_led = request_status_lotacao_A_v('localhost', 10399)
    if estacionamento_lotado or verifica_led:
        sinal_de_lotado_fechado.on()
    else:
        sinal_de_lotado_fechado.off()

    request_status_lotacao_A('localhost', 10399)
    
    print(f"Vaga {tipo} A{vaga_num} {estado_str}.")
    
def primeiro_fechar_estacionamento():
    global estacionamento_lotado
...
def primeiro_abrir_estacionamento():
...
def carro_descendo(vaga_num):
    ...
    controlar_saida('oi')
    #send_data('localhost', 10399, to_json_message(message_descendo))
...
def remover_carro_por_vaga_primeiro(vaga_num):
    ...
    if (('A' in vaga_info) and (car['dados']['vaga_num'] == vaga_num)):
        atualizar_current_charge(car)
        send = car['dados']['id']
        request_remove_car('localhost', 10399, send)
        sleep(0.5)
